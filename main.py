...
def siamese_model(model, input1, num, layers):
  ...
  model = Model(inputs=inputs, outputs=predF2)
...
if __name__ == '__main__':
  ...
  mc = ModelCheckpoint(f2, monitor='val_acc', mode='max', save_best_only=True)
  callbacks_list = [lrate, es, mc]

  trnGen = SiameseSequence(trn, train_augs)
  valGen = SiameseSequence(val)
  tstGen = SiameseSequence(tst)
  ...
  test_report('validation_shape',siamese_net, valGen)
